...
function isSortableItemElement(
  child: React.ReactNode,
): child is React.ReactElement<SortableItemProps> {
  return React.isValidElement(child) && child.type === SortableItem;
}
...
function Sortable<T>({
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <SortableContext items={itemIds} strategy={activeStrategy}>
        ...
        {activeId ? (
          <div className="z-50">
            {React.Children.map(children, (child) => {
              if (isSortableItemElement(child) && child.props.value === activeId) {
