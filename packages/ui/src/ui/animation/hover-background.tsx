...
type AnimatedObjectProps = {
  obj: AnimatedObjectData;
  springX: ReturnType<typeof useSpring>;
  springY: ReturnType<typeof useSpring>;
  glow: string;
};

function AnimatedObject({ obj, springX, springY, glow }: AnimatedObjectProps) {
  const shouldReduceMotion = useReducedMotion();
  const x = useTransform(springX, (v) => v * obj.parallaxStrength);
  const y = useTransform(springY, (v) => v * obj.parallaxStrength);
  ...
  return (
    <motion.div
      ...
      animate={
        shouldReduceMotion
          ? { scale: 0.7, opacity: 0.5, rotate: obj.baseRotation }
          : {
              scale: [0.6, 0.8, 0.6],
              opacity: [0.4, 0.6, 0.4],
              rotate:
...
function HoverBackground({
  ...
  const {
    background = "bg-linear-to-br from-slate-900 via-purple-900 to-slate-900",
    objects = [
      "bg-cyan-400/20",
      "bg-purple-400/20",
      "bg-fuchsia-400/20",
  ...
  return (
    <motion.div
      ...
      animate={
        shouldReduceMotion ? undefined : { backgroundPosition: ["0% 50%", "100% 50%", "0% 50%"] }
      }
      style={{
    ...
    >
      {/* Subtle ambient glow */}
      <motion.div
        className="absolute inset-0 bg-radial from-white/5 via-transparent to-transparent"
        animate={{
          opacity: [0.3, 0.6, 0.3],
          scale: [1, 1.1, 1],
        }}
      ...
      {mounted &&
        animatedObjects.map((obj) => (
          <AnimatedObject key={obj.id} obj={obj} springX={springX} springY={springY} glow={glow} />
        ))}
