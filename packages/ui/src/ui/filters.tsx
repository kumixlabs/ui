...
interface OptionWithIcon<T = unknown> {
  value: T;
  label: string;
  icon?: React.ReactNode;
}

function renderOptionCommandItems<T>({
  selectedOptions,
  unselectedOptions,
  onSelectSelected,
  onSelectAvailable,
  ...
  unselectedOptions: OptionWithIcon<T>[];
        ...
        <CommandGroup>
          {selectedOptions.map((option) => (
            <CommandItem
              ...
              onSelect={() => onSelectSelected(option.value)}
            >
          ...
          <CommandGroup>
            {unselectedOptions.map((option) => (
              <CommandItem
                ...
                onSelect={() => {
                  if (isMultiSelect) {
                    onSelectAvailable(option.value);
                  } else {
                    onSelectAvailable(option.value);
                    onClose?.();
                  }
      ...
      <div className="w-full">
        <Command>
          ...
          <CommandList>
            ...
            {renderOptionCommandItems({
              selectedOptions,
              unselectedOptions,
              onSelectSelected: (value) => {
                const change = field.onValueChange || onChange;
                if (isMultiSelect) {
                  change(effectiveValues.filter((v) => v !== value) as T[]);
                } else {
                  change([] as T[]);
                }
              },
              onSelectAvailable: (value) => {
                const change = field.onValueChange || onChange;
                if (isMultiSelect) {
                  const newValues = [...effectiveValues, value] as T[];
                  if (field.maxSelections && newValues.length > field.maxSelections) return;
                  change(newValues);
                } else {
                  change([value] as T[]);
                }
        ...
        <Command>
          ...
          <CommandList>
            ...
            {renderOptionCommandItems({
              ...
              onSelectSelected: (value) => {
                if (isMultiSelect) {
                  onChange(values.filter((v) => v !== value) as T[]);
                } else {
                  onChange([] as T[]);
                }
              },
              onSelectAvailable: (value) => {
                if (isMultiSelect) {
                  const newValues = [...values, value] as T[];
                  if (field.maxSelections && newValues.length > field.maxSelections) return;
                  onChange(newValues);
                } else {
                  onChange([value] as T[]);
