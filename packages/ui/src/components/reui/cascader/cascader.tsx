...
import type { CascaderGetChildren, CascaderOnSearch, CascaderResolveValue } from "./cascader-async";
import { useCascaderLoader, useCascaderLoadState } from "./cascader-async";
import type {
...
import { resolveCascaderLabels } from "./cascader-i18n";
import type { CascaderCheckedStrategy } from "./cascader-lib";
import {
...
function Cascader<T>({
   ...
   */
  const [_virtualSyncs, setVirtualSyncs] = React.useState(0);
...
function CascaderChips({
  ...
  return (
    ...
    >
      {content ??
        (nodes.length === 0 ? (
          <span data-slot="cascader-chips-placeholder" className="truncate text-muted-foreground">
            {placeholder}
          </span>
        ) : (
          nodes.map((node) => (
      ...
      >
        <ComboboxPrimitive.Popup
          ...
          className={cn(
            ...
            "group/combobox-content relative flex max-h-(--available-height) min-w-(--anchor-width) max-w-(--available-width) origin-(--transform-origin) flex-col",
            className,
          )}
          {...props}
        />
