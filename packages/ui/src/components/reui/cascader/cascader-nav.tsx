...
import type { CascaderCollapse, CascaderNode, CascaderValueDisplay } from "./cascader-types";
...
function CascaderNav({ className, ...props }: CascaderNavProps) {
  const defaultProps = {
    ...
    className: cn(
      ...
      "flex shrink-0 flex-col gap-1 border-border/60 border-b px-1.5 py-1",
      className,
    ),
  };
...
function CascaderBreadcrumb({
  ...
  const defaultProps = {
    ...
    className: cn(
      "flex min-w-0 shrink-0 items-center gap-0.5 pt-1.5 pb-0.5 text-muted-foreground text-xs",
      ...
      <React.Fragment key={segment.type === "node" ? segment.node.value : `gap-${i}`}>
        ...
        {segment.type === "ellipsis" ? (
          ...
          <span
            ...
            className={cn(
              "max-w-32 truncate",
              segment.node.value === currentValue && "font-medium text-foreground",
            )}
          >
            {segment.node.label}
          </span>
...
function CascaderValue({
  ...
  if (children) {
    ...
    content = <span className="truncate text-muted-foreground">{placeholder}</span>;
...
export { CascaderBack, CascaderBreadcrumb, CascaderInput, CascaderNav, CascaderValue };
