  ...
  return (
    ...
    >
      <AutocompletePrimitive.List
        ...
        className={cn(
          "in-data-has-overflow-y:me-3 not-empty:scroll-py-1 not-empty:px-1 not-empty:py-1",
          className,
        )}
        {...props}
      />
      ...
      >
        <div className="relative flex max-h-full">
          <AutocompletePrimitive.Popup
            ...
            className={cn(
              "flex max-h-[min(var(--available-height),24rem)] w-(--anchor-width) max-w-(--available-width) origin-(--transform-origin) scroll-pt-2 scroll-pb-2 flex-col overscroll-contain rounded-lg bg-popover py-0.5 text-popover-foreground shadow-md ring-1 ring-foreground/10 transition-[scale,opacity] has-data-[side=none]:scale-100 has-data-starting-style:scale-98 has-data-starting-style:opacity-0 has-data-[side=none]:transition-none",
              className,
            )}
            {...props}
          >
...
function AutocompleteGroupLabel({
  ...
  return (
    <AutocompletePrimitive.GroupLabel
      ...
      className={cn("px-1.5 py-1 font-medium text-muted-foreground text-xs", className)}
      {...props}
    />
  );
}
