...
import { FilterFieldPicker } from "./filters-builder";
import {
...
import { FILTER_ROW_SELECTOR, useFilterRowDrag } from "./filters-dnd";
import { filterIssueLabel } from "./filters-i18n";
import {
...
import { getFilterArity, getFilterOperator, operatorTakesValue } from "./filters-operators";
import { collectFilterIssues, createFilterRule, isFilterRule } from "./filters-query";
import type {
  FilterCombinator,
...
const COMBINATOR_TEXT_CLASS = cn(
  "w-full truncate px-1 text-center text-muted-foreground text-sm",
  SERA_TEXT_CLASS,
);
...
const ACTION_BAND_CLASS = cn("flex shrink-0 items-center gap-0.5 pe-1 text-muted-foreground");
    ...
    <>
      ...
      renderEmptyState ? (
        ...
        >
          ...
          <p className="font-medium text-sm">{actions.labels.builderEmpty}</p>
          ...
          trigger ? (
            ...
            >
              ...
              {ruleCount > 0 ? (
                <span
                  aria-hidden="true"
                  className="rounded-sm bg-muted px-1.5 text-muted-foreground text-xs tabular-nums"
                >
                  {ruleCount}
                </span>
              ) : null}
