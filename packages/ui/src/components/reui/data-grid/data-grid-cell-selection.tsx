...
function startDataGridFillSession<TData extends object>(options: {
  ...
  const finish = (commit: boolean) => {
    ...
    const targets = getExtensionTargets();
    if (!targets?.rowEntries.length) return;
...
function DataGridCellSelection<TData extends object>({
  ...
  useEffect(() => {
    ...
    keyboard,
    clipboard,
    table.atoms.cellSelection?.subscribe,
    context.props.tableLayout?.cellEditMode,
    context.table,
    context.props.onCellsCopy,
    context.props.onCellEditRequest,
    context.props.tableLayout?.cellSelectionMode,
    context.props.onCellsChange,
    context.props.onCellSelectionChange,
    apiRef,
  ]);
