...
function bubbleContentClass(variant: MessageBubbleVariant, interactive: boolean) {
  return cn(
    "relative z-0 min-w-9 max-w-[82%] rounded-2xl px-3.5 py-2.5 text-foreground text-sm leading-6",
    ...
    >
      <div
        ...
        className={cn(
          ...
          !currentOpen && "mask-[linear-gradient(to_bottom,#000_68%,transparent_100%)]",
          contentClassName,
        )}
      >
        {children}
      ...
      <button
        ...
        className={cn(
          "mt-2 inline-flex h-7 items-center gap-1 rounded-full px-2 font-medium text-muted-foreground text-xs outline-none transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring",
          triggerClassName,
        )}
      >
