...
export type FileUploadOptions = {
  ...
  /** Callback function executed when validation errors occur */
...
export const useFileUpload = (
  ...
  const addFiles = useCallback(
    (newFiles: FileList | File[]) => {
      ...
      setState((prev) => {
        const baseFiles = multiple ? prev.files : [];
        ...
        if (
          multiple &&
          maxFiles !== Number.POSITIVE_INFINITY &&
          baseFiles.length + dedupedFiles.length > maxFiles
        ) {
          const nextErrors = [`You can only upload a maximum of ${maxFiles} files.`];
          onError?.(nextErrors);
        ...
        const nextFiles = !multiple ? dedupedFiles : [...baseFiles, ...dedupedFiles];
        if (dedupedFiles.length > 0) {
          onFilesAdded?.(dedupedFiles);
