...
// Singleton instances for common observer options to maximize reuse.
const observers = new Map<string, SharedObserver>();
const rootIds = new WeakMap<Element, number>();
let rootId = 0;

function getRootKey(root: Element | Document | null | undefined) {
  if (!root) return "null";
  if (!rootIds.has(root as Element)) {
    rootIds.set(root as Element, ++rootId);
  }
  return rootIds.get(root as Element);
}
...
function getSharedObserver(options: IntersectionObserverInit) {
  const threshold = Array.isArray(options.threshold)
    ...
    : (options.threshold ?? 0);
  const key = `${getRootKey(options.root)}|${options.rootMargin ?? "0px"}|${threshold}`;
  if (!observers.has(key)) {
    observers.set(key, new SharedObserver(options));
  }
  return { key, observer: observers.get(key)! };
}
...
export function useIntersectionObserver(
  ...
  React.useEffect(() => {
    ...
    const { key, observer } = getSharedObserver({ threshold, root, rootMargin });
