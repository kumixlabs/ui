...
Same names can exist in multiple categories (e.g. `alert`, `badge`, `checkbox`). `find_component` and `get_usage_example` return **all** matches (`matches[]` + `importPath` per entry).
